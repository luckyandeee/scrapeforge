...
// const querySearchCluster = async (campaignName: string, profession: string, specificLocation: string, lowPowerMode: boolean) => {
//   const queryVariations = await generateQueryMatrix(profession, specificLocation, lowPowerMode);
...
//             await scrapeEngine(engineContext, engine.name, queryStr, campaignName, engine.url, specificLocation, profession, true);
//           } catch (e) {
//           } finally {
...
//             engineBrowser = cleanSession.browser;
//             engineContext = cleanSession.context;
//             await scrapeEngine(engineContext, engine.name, queryStr, campaignName, engine.url, specificLocation, profession, false);
//           } catch (e) {
//           } finally {
//             if (engineContext) { await engineContext.close().catch(() => {}); engineContext = null; }
//             if (engineBrowser) { await engineBrowser.close().catch(() => {}); engineBrowser = null; }
...
//   // 🚀 AI FAILSAFE: Wrap in try/catch to prevent OpenAI timeouts from crashing script
//   try {
