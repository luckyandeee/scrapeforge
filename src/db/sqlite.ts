...
// 🚀 BULLETPROOF APPDATA RESOLVER: Uses os.homedir() to guarantee it never hits the desktop
export const getAppDataDir = (): string => {
  ...
  const safeFolder = path.join(appData, 'ScrapeForge');
...
// 🚀 HIDDEN BACKEND LOGGER: Silently stores logs inside AppData/ScrapeForge out of sight
export const writeDebug = (msg: string) => {
  try {
    const logPath = path.join(getAppDataDir(), 'ScrapeForge_Backend_Debug.txt');
    fs.appendFileSync(logPath, `[${new Date().toISOString()}] ${msg}\n`);
  } catch (e) {}
};

writeDebug("1. SQLite Module Initializing...");
const dbPath = path.join(getAppDataDir(), 'scrapeforge.db');
writeDebug(`2. Database Path Resolved: ${dbPath}`);
...
// 1. Create Core Tables (Unified & Optimized for Local-First)
db.exec(`
...
export const queries = {
  ...
  // 🚀 FULL PURGE HELPER (Wipes database clean - Essential for Data Principals)
  purgeAllData: db.transaction(() => {
      db.prepare(`DELETE FROM businesses`).run();
      db.prepare(`DELETE FROM crawl_state`).run();
      db.prepare(`DELETE FROM engine_state`).run();
  }),
...
// 🚀 UNIFIED METRIC SYSTEM: Accurate counting that respects user filters across ALL engines.
export const getHighValueCount = (campaignName: string, requirement: string = "any"): number => {
  ...
  if (requirement === "phone") condition = `AND ${hasPhone}`;
  if (requirement === "email") condition = `AND ${hasEmail}`;
  if (requirement === "both") condition = `AND ${hasPhone} AND ${hasEmail}`;
  // Only count leads that actually successfully met the requirement and are marked processed
  return db.prepare(`
    SELECT COUNT(id) FROM businesses 
    WHERE campaign_name LIKE ? AND status = 'processed' ${condition}
...
writeDebug("4. Database Storage Layer Online.");
console.log('✅ Local Database storage layer online and schema verified.');