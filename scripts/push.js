...
async function main() {
  ...
  try {
    ...
    if (shouldBump) {
      ...
      newVersion = `v${pkg.version}`;

      console.log(`📝 Syncing version ${newVersion} into README.md...`);
      if (fs.existsSync('./README.md')) {
        let readmeContent = fs.readFileSync('./README.md', 'utf8');
        
        // 1. Update standard tag links pointing to the release page
        readmeContent = readmeContent.replace(/releases\/tag\/v[\d\.]+/g, `releases/tag/${newVersion}`);
        
        // 2. Update dynamic shield badges (e.g. Release-v1.4.0-blue)
        readmeContent = readmeContent.replace(/Release-v[\d\.]+-/g, `Release-${newVersion}-`);
    ...
    if (shouldBump) {
      console.log(`🏷️ Pushing release tag ${newVersion} to GitHub...`);
      // 🚀 CRITICAL FIX: Only push the exact tag we just created, NOT --tags
      await runCommand(`git push origin ${newVersion}`, `Tag ${newVersion} pushed`);
      console.log('☁️ GitHub Actions triggered! Cloud pipeline is now building installers and publishing the release automatically.');
    }

    console.log('\x1b[32m\x1b[1m🎉 All done! Code pushed and release pipeline triggered successfully!\x1b[0m\n');
